...
For the initial version of the exhibition, a harpsichord sample library was used and controlled via Native Instrument's Kontakt. The software was installed on a Mac Mini hosted inside the instrument's case. Holes were drilled at the back of the instrument's frame, away from sight, allow the passage of cables, such as from power supplies, USB connectors and headphone jack (Figure \ref{fig:mac-mini}). 
% These components are accessible via locked drawer which is visible in \ref{fig:oratory}. 
An iPad is used as a monitor through which visitors can adjust parameters of playback such as tuning, voicing and which stops are engaged. The next iteration will use a library of samples from instruments in the collection, which is currently being carried out by the museum.
...
% Testers unfamiliar with harpsichords were particularly impressed by its responsiveness.
